 ...
 * 【更新日志】
 * 2021/02/28 1.0.0 发布
 * 2021/03/03 1.0.1 修正默认色调问题并校准窗口高度
 * 2021/03/05 1.0.2 修复未使用冲刺速度或快速消息时的错误
 * 2021/04/05 1.1.0 添加 A/B 按钮切换与用于禁用标准项目的参数
 * 2021/06/22 1.2.0 新增多项参数并兼容子文件夹
 * 2021/07/06 1.2.1 支持 RMMZ 1.3.2 的子文件夹改进
 * 2022/02/20 1.2.2 支持 NovelGameUI.js
 * 2022/02/21 1.2.3 改善与其他提供选项的插件之间的冲突
 * 2022/02/24 1.3.0 新增将窗口放在按钮下方的参数，并重写说明
 * 2022/03/03 1.3.1 修复新游戏时默认值未被应用的问题
 * 
 * 本插件扩展了选项场景。
 * 功能不仅包括更改选项窗口的外观、为音量等项目显示进度条，
 * 还可新增以下选项项目：
 * 
 * 1. A/B 按钮切换
 * 2. 快速消息
 ...
 * 
 * 另外会添加“恢复为默认值”的按钮。
 * 对于数值与进度条类选项，玩家可使用 PageUp/PageDown（支持触摸 UI 按钮）
 * 来大幅调整数值。对于进度条，可拖动/滑动来改变数值。
 * 你可以选择是否启用各个选项项目。
 * 
 * 提供了多个与选项窗口尺寸/位置相关的参数。
 * 若因项目过多导致无法完全显示，请调整这些参数。
 * “选项项目高度”决定每个项目的高度；调低该值可增加可见项目数量。
 * 若项目文字超出边框，也请调低“字体大小”。“默认命令偏移”表示
 * 最下方项目与“恢复为默认值”按钮之间的距离。将“在按钮下方显示选项窗口”
 * 设为 true 时，窗口会放在触摸 UI 按钮区域之下，此时窗口高度会变小，
 * 可相应调低上述参数以确保全部项目可见。
 * 
 * 如果将参数“在移动设备上隐藏触摸 UI 选项”设为 true（默认 true），
 * 则在平板/智能手机上不会显示“触摸 UI”选项。
 * 这样可避免玩家误将其关闭后菜单按钮消失，必须双指点按才能打开菜单的问题。
 * 
 * 1. A/B 按钮切换
 * 打开后，手柄的确定/取消按钮将对调。若参数“未连接手柄时隐藏 A/B 按钮切换”
 * 为 true（默认 true），在未连接手柄时不会显示该选项。
 * 
 * 2. 快速消息
 * 打开后，消息窗口中文本将即时显示。
 * 
 * 3. 冲刺速度
 * 可选择 3 档速度。“1”与 MZ 默认速度相同。
 * 
 * 4. 窗口皮肤
 * 提供可替换的窗口皮肤。要使用该项目，请将窗口图片文件（原名为“Window.png”，
 * 也可使用任意文件名）放入 img/system 文件夹，然后在插件参数“窗口皮肤图片”
 * 中添加它们。若该参数中仅有一个图片文件，则不会显示此选项项目。
 * 
 * 5. 窗口颜色
 * 更改窗口的色调。会显示进度条，玩家可拖动/滑动进行调整。
 * 
 * 6. 窗口不透明度
 * 更改窗口的不透明度。会显示进度条，玩家可拖动/滑动进行调整。
 * 
 * 本插件以 MIT 许可发布。
 * https://opensource.org/licenses/mit-license.php
 *
 ...
 * @param useTouchUI
 * @text 启用“触摸 UI”
 * @desc 是否启用用于切换触摸 UI 的选项。
 * @parent touchUI
 * @default true
 * @type boolean
 * 
 * @param hideTouchUIForMobiles
 * @text 在移动设备上隐藏“触摸 UI”选项
 * @desc 当玩家使用智能手机或平板时，是否隐藏“触摸 UI”选项。
 * @parent touchUI
 * @default true
 * @type boolean
 * 
 ...
 * @param switchABButtons
 * @text A/B 按钮切换
 * @desc “A/B 按钮切换”选项的相关设置。
 * 
 * @param useSwitchABButtons
 * @text 启用 A/B 按钮切换
 * @desc 是否启用用于切换 A/B 按钮的选项。
 * @parent switchABButtons
 * @default true
 * @type boolean
 * 
 * @param hideSwitchABButtonsIfGamepadUnconnected
 * @text 未连接手柄时隐藏 A/B 按钮切换
 * @desc 若未连接手柄，是否隐藏 A/B 按钮切换选项。
 * @parent switchABButtons
 * @default true
 * @type boolean
 * 
 ...
 * @param defaultDashSpeed
 * @text 默认“冲刺速度”
 * @desc 指定冲刺速度的默认值（0–2）。注意：在界面上会显示为 +1。
 * @parent dashSpeed
 * @default 0
 * @type number
 * @min 0
 ...
 * @param defaultWindowOpacity
 * @text 默认“窗口不透明度”
 * @desc 窗口不透明度的默认值。RMMZ 1.3.0 及以上版本可不设置。
 * @parent windowOpacity
 * @default 195
 * @type number
 * @min 0
